...
**Validation.** gRPC APIs use Protocol Buffer encoding, so they are more observable, and API management systems can look inside messages to validate or monitor requests and responses.

**Metadata.** Protocol Buffer encoding isn't self-describing, so API management systems can make it easier to use gRPC APIs by providing standard metadata services.

**Documentation.** gRPC APIs are well-defined, so API management systems can easily provide generated documentation and even API client code.

**Transcoding.** HTTP/JSON transcoding is well-defined, so API management systems can provide transcoded versions of gRPC APIs that they manage.

**Transformation.** Projects like [rejoiner](https://google.github.io/rejoiner/) show that gRPC APIs can be combined to create aggregated GraphQL APIs. This could be part of an advanced gRPC API management system.
