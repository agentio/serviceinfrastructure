...
Meanwhile, the Cloud Run product advanced to allow [sidecar deployments](https://cloud.google.com/blog/products/serverless/cloud-run-now-supports-multi-container-deployments), which we demonstrate for our demo example. With this, it's possible to deploy a service and a proxy side-by-side in a single deployment with a [unified YAML configuration file](/docs/quickstart/files#serviceyaml). With this, the proxies conveniently blend into service deployments and require no explicit management, but their logs and metrics are available, as always, in the Cloud Endpoints console. This is a better developer experience than what API Gateway offers and a strong benefit to using Cloud Run. Also, these Cloud Run deployments are very similar to proxy deployments in Kubernetes, making it easy to move from Cloud Run to more general Kubernetes platforms.
