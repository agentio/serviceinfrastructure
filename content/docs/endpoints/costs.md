...
As you might imagine, the number of calls you make to the Service Management API is relatively low, and Google does not charge for them. On the other hand, the number of calls that you make to Service Control can be quite high, generally corresponding to the number of API calls that your users make, but these calls cost very little. Google charges based on the number of API operations being tracked: in other words, the number of calls to your API. The first two million calls per month are free, after that they cost $3/million, and after a billion calls, the price drops to $1.5/million. To make this more tangible, the table below shows a few examples.

| calls per month | (per second) | cost per month | 
| --------------- | ------------ | -------------- |
| under 2 million | <1 | $0 |
| 3 million | ~1 | $3 |
| 10 million | ~4 | $24 |
